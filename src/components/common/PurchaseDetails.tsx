...
const PurchaseDetails = ({ handleButtonPress, buttonLabel, cartItemToEdit }: PurchaseDetailsProps) => {
  ...
  const addToCart = () => {
    ...
    if(
      ...
      addItem({id: id, quantity: quantity, amount: amount, name: giftCard!.name, giftCard: giftCard!, email: email.value, phone: phone.value, note: note.value, otherAmount: otherAmount.value});
      addItemToEdit({});
      resetForm();
      handleButtonPress();
    }
